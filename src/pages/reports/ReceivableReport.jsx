...
export default function ReceivableReport({ company }) {
  ...
  const run = async () => {
    ...
    try {
      const result = await reportsService.getAccountsReceivable({ company, reportDate });
      setData(result);
    } catch (err) {
  ...
  const totals = rows.reduce((acc, row) => {
    if (!row.party) return acc; // ignorer les lignes vides
  ...
  return (
    <div>
      ...
      {!data && !loading && (
        <EmptyState message="Cliquez sur Générer pour afficher la balance âgée clients" />
      )}
      ...
      {!loading && data && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            {[
              { label: 'Total encours', value: totals.outstanding, color: 'text-gray-900' },
          ...
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100">
              <h3 className="text-sm font-semibold text-gray-900">
                Balance âgée clients — {dataRows.length} entrée{dataRows.length > 1 ? 's' : ''}
              </h3>
            </div>
            {dataRows.length === 0 ? (
              <p className="text-center py-8 text-sm text-gray-400">Aucune créance en attente</p>
            ) : (<>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {displayRows.map((row, i) => (
                      <tr key={i} className="hover:bg-gray-50/30">
                        <td className="px-4 py-2 font-medium text-gray-900">{row.party || row.party_name || '—'}</td>
