  ...
  const loadCustomers = useCallback(async () => {
    ...
    try {
      const [data, count] = await Promise.all([
        ...
        customerService.getCount({ search }),
      ]);
      setCustomers(data || []);
      setTotal(count || 0);
    } catch (err) {
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">{total} client{total > 1 ? 's' : ''} au total</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200">
          <EmptyState
            ...
            description={search ? `Aucun résultat pour "${search}"` : 'Commencez par créer votre premier client.'}
            action={!search ? () => navigate('/clients/nouveau') : undefined}
                ...
                {customers.map((c) => (
                  <tr key={c.name} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-3">
                      <Link to={`/clients/${encodeURIComponent(c.name)}`} className="hover:text-cobilan-600 transition">
                    ...
                    <td className="px-6 py-3 hidden md:table-cell">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                        c.customer_type === 'Company' ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-600'
                      }`}>
                        {c.customer_type === 'Company' ? 'Entreprise' : 'Particulier'}
                      </span>
                    </td>
                    <td className="px-6 py-3 text-gray-600 hidden lg:table-cell">{c.mobile_no || '—'}</td>
                    <td className="px-6 py-3 text-gray-600 hidden lg:table-cell">{c.email_id || '—'}</td>
                    <td className="px-6 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => navigate(`/clients/${encodeURIComponent(c.name)}/modifier`)}
                          className="p-1.5 text-gray-400 hover:text-cobilan-600 rounded-lg hover:bg-cobilan-50 transition" title="Modifier">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z" />
                          </svg>
                        </button>
                        <button onClick={() => setDeleteTarget(c.name)}
                          className="p-1.5 text-gray-400 hover:text-red-600 rounded-lg hover:bg-red-50 transition" title="Supprimer">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <ConfirmModal
        ...
        message="Cette action est irréversible. Le client ne pourra être supprimé que s'il n'a aucune facture liée."
