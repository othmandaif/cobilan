...
export default function CustomerDetail() {
  ...
  const loadAll = async () => {
    setLoading(true);
    try {
      const [cust, inv, addr, cont] = await Promise.all([
        customerService.getById(id),
        customerService.getInvoices(id),
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(parseERPNextError(err) || 'Impossible de supprimer ce client.');
    } finally {
      setDeleting(false);
      setShowDelete(false);
    }
  ...
  if (!customer) {
    return (
      <div className="text-center py-12">
        ...
        <Link to="/clients" className="mt-4 inline-block text-sm text-cobilan-600 hover:underline">
          Retour aux clients
        </Link>
      </div>
    );
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => navigate(`/clients/${encodeURIComponent(id)}/modifier`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <dl className="space-y-3">
              <InfoRow label="Type" value={customer.customer_type === 'Company' ? 'Entreprise' : 'Particulier'} />
              <InfoRow label="Groupe" value={customer.customer_group} />
              <InfoRow label="Territoire" value={customer.territory} />
              <InfoRow label="Téléphone" value={customer.mobile_no} />
              <InfoRow label="Email" value={customer.email_id} />
              <InfoRow label="ICE / IF" value={customer.tax_id} />
              <InfoRow label="Site web" value={customer.website} />
              <InfoRow label="Devise" value={customer.default_currency || 'MAD'} />
            </dl>
          </div>

          {/* Encours */}
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <h3 className="text-sm font-semibold text-gray-900 mb-2">Encours</h3>
            <p className={`text-2xl font-bold ${
              (customer.outstanding_amount || 0) > 0 ? 'text-orange-600' : 'text-green-600'
            }`}>
              {formatMAD(customer.outstanding_amount)}
            </p>
          </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl border border-gray-200">
            ...
            {invoices.length === 0 ? (
              <div className="p-6">
                <EmptyState icon="invoices" title="Aucune facture" description="Aucune facture de vente pour ce client." />
              </div>
            ) : (
      ...
      <ConfirmModal
        ...
        message={`Êtes-vous sûr de vouloir supprimer "${customer.customer_name}" ? Cette action est irréversible.`}
