...
export default function PurchaseInvoiceDetail() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await purchaseInvoiceService.submit(id);
      toast.success('Facture d\'achat soumise avec succès');
      setShowSubmit(false);
      loadInvoice();
    } catch (err) {
  ...
  const handleCancel = async () => {
    ...
    try {
      await purchaseInvoiceService.cancel(id);
  ...
  if (loading) return <PageLoader message="Chargement de la facture d'achat…" />;

  if (!invoice) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">Facture introuvable</p>
        <Link to="/factures-achat" className="mt-4 inline-block text-sm text-cobilan-600 hover:underline">
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
          <button
            onClick={() => navigate('/factures-achat')}
        ...
        <div className="flex flex-wrap gap-2">
          {/* Télécharger PDF */}
          <button
            onClick={() => window.open(purchaseInvoiceService.getPdfUrl(invoice.name), '_blank')}
          ...
          {isDraft && (
            <button
              onClick={() => navigate(`/factures-achat/${encodeURIComponent(id)}/modifier`)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <dl className="space-y-3">
              <InfoRow label="Fournisseur" value={invoice.supplier_name} />
              <InfoRow label="Société" value={invoice.company} />
              ...
              {invoice.bill_no && <InfoRow label="N° facture fournisseur" value={invoice.bill_no} />}
              {invoice.bill_date && <InfoRow label="Date facture fournisseur" value={formatDate(invoice.bill_date)} />}
            </dl>
          </div>
        ...
        <div className="lg:col-span-1">
          <ActivityTimeline doctype="Purchase Invoice" name={id} />
        </div>
      </div>
