...
function StatusBadge({ status }) {
  const styles = {
    ...
    'Debit Note Issued': 'bg-indigo-50 text-indigo-700',
  };
  const labels = {
    Draft: 'Brouillon',
    Submitted: 'Soumise',
    ...
    Cancelled: 'Annulée',
    'Return': 'Avoir',
    'Debit Note Issued': 'Note de débit',
  };
  return (
...
export default function PurchaseInvoiceList() {
  ...
  const loadInvoices = useCallback(async () => {
    ...
    try {
      const [data, count] = await Promise.all([
        purchaseInvoiceService.getAll({ page, limit, status: statusFilter, search }),
        purchaseInvoiceService.getCount({ status: statusFilter }),
      ]);
      setInvoices(data || []);
      setTotal(count || 0);
    } catch (err) {
      toast.error('Impossible de charger les factures d\'achat');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await purchaseInvoiceService.delete(deleteTarget);
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Factures d'achat</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200">
          <EmptyState
            ...
            title={search || statusFilter ? 'Aucune facture trouvée' : 'Aucune facture d\'achat enregistrée'}
            description={search || statusFilter ? 'Modifiez vos filtres ou votre recherche.' : 'Commencez par créer votre première facture d\'achat.'}
            action={!(search || statusFilter) ? () => navigate('/factures-achat/nouvelle') : undefined}
                ...
                {invoices.map((inv) => (
                  <tr key={inv.name} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-3">
                      <Link to={`/factures-achat/${encodeURIComponent(inv.name)}`} className="font-medium text-cobilan-600 hover:text-cobilan-800">
                        {inv.name}
                      </Link>
                    </td>
                    <td className="px-6 py-3 text-gray-900">{inv.supplier_name || inv.supplier}</td>
                    ...
                    <td className="px-6 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {/* PDF */}
                        <button
                          onClick={() => window.open(purchaseInvoiceService.getPdfUrl(inv.name), '_blank')}
